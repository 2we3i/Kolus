// ==UserScript==
// @name         Автозаполнение опор - v1.8.4 (KOLUS Style)
// @namespace    pillar.autofill.advanced
// @version      1.8.4
// @description  🏗️ Заполнение формы опор в стиле KOLUS | Created by KAST Team
...
(function() {
    ...
    console.log('🚀 Автозаполнение опор v1.8.4 загружен | Created by KAST Team');
    ...
    const PILLAR_TYPE_TO_MATERIAL = {
        'СВ 95': 'железобетонная', 'СВ 110': 'железобетонная', 'СКЦ': 'железобетонная',
        'Деревяная н/в': 'деревянная', 'ОСЦ-1.5-9.0': 'металлическая', 'Трубостойка': 'металлическая'
    };
    const PILLAR_TYPES       = ['СВ 95', 'СВ 110', 'СКЦ', 'Деревяная н/в', 'ОСЦ-1.5-9.0', 'Трубостойка'];
    const CONSTRUCTION_TYPES = [
        { name: 'одностоечная', stands: 1 }, { name: 'двухстоечная', stands: 2 },
        { name: 'трехстоечная', stands: 3 }, { name: 'А-образная', stands: 2 }, { name: 'П-образная', stands: 2 }
    ];
    const PLACEMENT_TYPES = ['Ответвительная', 'Промежуточная', 'Угловая', 'Концевая'];
    const BRACING_MATS    = ['деревянная', 'металлическая', 'железобетонная'];
    const ATTACH_MATS     = ['железобетонная', 'деревянная'];
    const LAMP_TYPES         = ['ДНАТ', 'ДРЛ', 'Светодиод', 'Ввод вручную'];
    const INSULATOR_TYPES    = ['поддерживающий', 'натяжной', 'штыревой', 'угловой'];
    const INSULATOR_MATS     = ['фарфоровый', 'стеклянный', 'полимерный'];

    const defaultData = {
        pillar: { type: 'СВ 95', constructionType: 'одностоечная', placement: 'Промежуточная', material: 'железобетонная' },
        stands: { count: 1, material: 'железобетонная' },
        bracing: { count: 0, material: 'металлическая' },
        attachments: { count: 0, material: 'железобетонная' },
        ...
        lamps: [],
        insulators: { count: 1, type: 'поддерживающий', material: 'фарфоровый', qty: 6 },
        wireTypeA: false,
    ...
    if (!db.attachments) db.attachments = { count: 0, material: 'железобетонная' };
    if (db.attachments.material !== 'железобетонная' && db.attachments.material !== 'деревянная') {
        db.attachments.material = 'железобетонная';
    }
    if (!db.subscribers) db.subscribers = { settlement: '', street: '' };
    if (!db.insulators) db.insulators = { count: 1, type: 'поддерживающий', material: 'фарфоровый', qty: 6 };
    ...
    async function fillField(selector, value, label) {
        ...
        if (!el) { console.warn(`${label}: не найдено`); return false; }
    ...
    /******************************************************************
     * КАРУСЕЛЬ (общая — для стоек, укосов, нагрузок)
     ******************************************************************/
    ...
    async function goToSubscriberSlide(target) {
        ...
        if (!carousel) {
            log('Карусель абонентов не найдена', 'warning');
            return false;
        }
        ...
        for (let attempt = 0; attempt < 6; attempt++) {
            ...
            // Запасной вариант — клик по точке навигации
            const dots = getDots();
            if (target < dots.length) {
    ...
    function buildDesiredLoads() {
        ...
        for (let i = 0; i < db.additionalLoad.streetLamp; i++) {
            const lamp = db.lamps[i] || { type: 'ДНАТ', customType: '', mounting: 'приставной', powerSupply: 'Кабель', direction: 'перпендикулярно линии' };
            list.push({ type: 'фонарь уличный', lamp });
        }
        for (let i = 0; i < db.additionalLoad.streetLantern; i++)
            list.push({ type: 'светильник' });
        for (let i = 0; i < db.additionalLoad.communicationCabinet; i++)
            list.push({ type: 'шкаф коммутационный', owner: 'Н/Д' });
        for (let i = 0; i < db.additionalLoad.communicationLine; i++)
            list.push({ type: 'линия связи', lineType: 'ВОЛС' });
        for (let i = 0; i < db.additionalLoad.volsMuffle; i++)
            list.push({ type: 'муфта ВОЛС' });
        for (let i = 0; i < db.additionalLoad.voltageLines; i++)
            list.push({ type: 'линии разных классов напряжения' });
        return list;
    }
    ...
    async function fillLoad(loadIndex, loadData) {
        ...
        await fillField(typeSelector, type, `Тип[${loadIndex}]`);
        await sleep(200);
        if (type === 'шкаф коммутационный') {
            await fillField(`input[name="additionalLoad.info[${loadIndex}].distributionCabinetOwner"]`, loadData.owner || 'Н/Д', `Владелец[${loadIndex}]`);
        } else if (type === 'линия связи') {
            await fillField(`input[name="additionalLoad.info[${loadIndex}].communicationLineType"]`, loadData.lineType || 'ВОЛС', `Тип линии[${loadIndex}]`);
        } else if (type === 'фонарь уличный' && loadData.lamp) {
            const lamp = loadData.lamp;
            const lt   = lamp.type === 'Ввод вручную' ? lamp.customType : lamp.type;
            await fillField(`input[name="additionalLoad.info[${loadIndex}].streetLampType"]`, lt, `Лампа[${loadIndex}]`);
            await sleep(150);
            ...
            if (powerInp) {
                ...
                if (powerMinus && powerPlus) {
                    ...
                    if (getVal() !== 3) log(`Мощность фонаря[${loadIndex}]: не удалось выставить 3 (осталось ${getVal()}), пропущено`, 'debug');
                }
            }
            await sleep(150);
            await fillField(`input[name="additionalLoad.info[${loadIndex}].streetLampMountType"]`, lamp.mounting, `Крепление[${loadIndex}]`);
            await sleep(150);
            await fillField(`input[name="additionalLoad.info[${loadIndex}].streetLampFasteningType"]`, lamp.powerSupply, `Питание[${loadIndex}]`);
            await sleep(150);
            await fillField(`input[name="additionalLoad.info[${loadIndex}].streetLampLuminaireDirection"]`, lamp.direction, `Направление[${loadIndex}]`);
            await sleep(150);
        ...
        } else if (type === 'светильник') {
            await clickCamera(`fieldset[name="additionalLoad.info[${loadIndex}].lampBracketPhotoUrls"] button.smwb-fab`);
        } else if (type === 'муфта ВОЛС') {
            await fillField(`input[name="additionalLoad.info[${loadIndex}].fiberOpticSpliceClosureOwner"]`, 'Н/Д', `Владелец муфты[${loadIndex}]`);
        }
        await sleep(150);
    ...
    async function syncInsulators() {
        ...
        log(`\n=== ИЗОЛЯТОРЫ (групп: ${ins.count}, шт.: ${ins.qty}) ===`, 'info');

        // Выставляем количество групп
        if (!await setCounter('input[name="insulator.count"]', ins.count)) {
            log('  ❌ Не удалось выставить количество групп', 'error');
            return;
        }
        await sleep(600);

        for (let i = 0; i < ins.count; i++) {
            log(`  Изолятор ${i + 1}/${ins.count}`, 'info');

            // Найти карусель изоляторов и переключить слайд
            const insCard = (() => {
            ...
            await fillField(`input[name="insulator.info[${i}].type"]`,     ins.type,     `Тип изолятора[${i}]`);
            await sleep(150);
            await fillField(`input[name="insulator.info[${i}].material"]`, ins.material, `Материал изолятора[${i}]`);
            await sleep(150);

            // Количество штук в группе
            await setCounter(`input[name="insulator.info[${i}].count"]`, ins.qty);
            await sleep(200);

            // Фидер — выбираем первый вариант только если ещё не выбран
            ...
            if (fiderFieldset) {
                ...
                if (!alreadySet && fiderSelect) {
                    ...
                    if (firstItem) {
                        ...
                        await sleep(200);
                        log(`  Фидер: выбран первый вариант`, 'success');
                    } else {
                        log(`  Фидер: варианты не найдены`, 'warning');
                    }
                } else if (alreadySet) {
                    log(`  Фидер: уже выбран "${currentVal.textContent.trim()}", пропущено`, 'debug');
                }
            }
            await sleep(150);
        ...
        log('  ✅ Изоляторы заполнены', 'success');
    }

    /******************************************************************
     * АБОНЕНТЫ — вспомогательные функции
     ******************************************************************/

    /******************************************************************
     * СИСТЕМА ПОСТОЯННОЙ ПОДСВЕТКИ ОШИБОК
     ******************************************************************/
    // Хранит CSS-селекторы полей с ошибками
    ...
    // Перебирает все зарегистрированные селекторы и подсвечивает найденные элементы
    function reapplyErrors() {
        for (const sel of errorSelectors) {
            const el = document.querySelector(sel);
            if (el) {
                const holder = el.closest('.smwb-text-field') || el.closest('fieldset') || el.parentElement;
                // Если holder уже имеет атрибут — уже подсвечен, не трогаем
    ...
    // Следим за изменениями DOM и переприменяем стили — это обеспечивает
    // восстановление подсветки после ре-рендера React (при листании карусели)
    const errorPersistObserver = new MutationObserver(() => {
        if (errorSelectors.size === 0) return;
        reapplyErrors();
        // Снимаем подсветку если форма закрылась
    ...
    // Зарегистрировать поле как ошибочное и сразу подсветить
    ...
    // Сбросить все ошибки (вызывается при новом запуске заполнения)
    ...
    // Выбрать первый вариант в smwb-select если значение пустое
    async function selectFirstIfEmpty(fieldsetSelector) {
        ...
        if (alreadySet) return true; // уже выбрано — не трогаем
    ...
    // Получить значение провода у абонента (поддерживает input и select)
    ...
    // Проверить провод по фазе
    // Фаза A/B/C → СИП 2х16, АВВГ 2х16, А-16
    // Фаза ABC   → СИП 4хY, АВВГ 4хY, А-Y  где Y > 16
    function validateCableForPhase(cable, phase) {
        ...
        if (singlePhase) {
            if (/^СИП 2х16$/i.test(cable))  return true;
            if (/^АВВГ 2х16$/i.test(cable)) return true;
            if (/^А-16$/.test(cable))        return true;
            return false;
        }
        if (threePhase) {
            let m;
            m = cable.match(/^СИП 4х(\d+)$/i);
            if (m && parseInt(m[1]) > 16) return true;
            m = cable.match(/^АВВГ 4х(\d+)$/i);
            if (m && parseInt(m[1]) > 16) return true;
            m = cable.match(/^А-(\d+)$/);
            if (m && parseInt(m[1]) > 16) return true;
            return false;
        }
        return true; // неизвестная фаза — не блокируем
    }
    ...
    async function syncSubscribers() {
        if (!db.subscribers.settlement && !db.subscribers.street) {
            log('Абоненты: поля пусты, пропущено', 'debug');
            return;
        }
        const countInp = document.querySelector('input[name="subscribers.count"]');
        if (!countInp) {
            log('Абоненты: форма не найдена', 'debug');
            return;
        }
        const total = parseInt(countInp.value) || 0;
        if (total === 0) {
            log('Абоненты: количество = 0', 'debug');
            return;
        }
        ...
        for (let i = 0; i < total; i++) {
            log(`Абонент ${i + 1}/${total}`, 'info');

            const moved = await goToSubscriberSlide(i);
            if (!moved) log(`  ⚠️ Не удалось перейти к слайду ${i}`, 'warning');
            await sleep(400);

            // Ждём появления полей в DOM (React рендерит слайды лениво)
            ...
            if (db.subscribers.settlement) {
                ...
                log(`  Населенный пункт: ${ok ? '✅' : '❌'} "${db.subscribers.settlement}"`, ok ? 'success' : 'warning');
                await sleep(150);
            }
            ...
            if (db.subscribers.street) {
                ...
                log(`  Улица: ${ok ? '✅' : '❌'} "${db.subscribers.street}"`, ok ? 'success' : 'warning');
                await sleep(150);
            }

            // ── ТП — выбрать первый если пустой ──────────────────────
            ...
            if (!tpVal) {
                ...
                log(`  ТП: ✅ "${tpVal}"`, 'success');
            }
            await sleep(150);

            // ── Фидер — выбрать первый если пустой ───────────────────
            ...
            if (!fiderVal) {
                ...
                log(`  Фидер: ✅ "${fiderVal}"`, 'success');
            }
            await sleep(150);

            // ── Способ крепления — всегда Натяжное ───────────────────
            await fillField(`input[name="subscribers.info[${i}].fastening.type"]`, 'Натяжное', `Крепление[${i}]`);
            await sleep(150);

            // ── Валидация провода по фазе ─────────────────────────────
            ...
            if (!cableValid || !cable) {
                ...
                const hint = singlePhase
                    ? 'Ожидается: СИП 2х16 / АВВГ 2х16 / А-16'
                    : threePhase
                        ? 'Ожидается: СИП 4хY / АВВГ 4хY / А-Y (Y > 16)'
                        : `Фаза: "${phase}" — неизвестная`;
                log(`  Провод: ❌ "${cable || 'пусто'}" (фаза "${phase}") — ${hint}`, 'warning');
                // Подсветить поле провода красным и перенести фокус на него
                ...
                // Небольшая пауза чтобы скролл успел отработать, затем фокус
                await sleep(400);
                if (cableInpEl) cableInpEl.focus();
            } else {
                log(`  Провод: ✅ "${cable}" (фаза "${phase}")`, 'success');
            }

            // ── Натяжение провода — заполняем ПОСЛЕ фокуса на проводе ─
            await sleep(150);
            const isTypeA = /^(А|АС)-\d+$/i.test(cable);
            const wireTension = isTypeA ? 'Вязка концевая' : 'Зажим натяжной клиновой';
            await fillField(`input[name="subscribers.info[${i}].wireTension"]`, wireTension, `Натяжение[${i}]`);
            log(`  Натяжение: "${wireTension}"`, 'debug');
            await sleep(150);
        }
    }

    /******************************************************************
     * АВТООПРЕДЕЛЕНИЕ ФОРМЫ
     ******************************************************************/
        ...
        try {
            ...
            if (loadCounter) {
                ...
                for (let i = 0; i < siteLoadCount; i++) {
                    ...
                    if (v === 'шкаф коммутационный') cabs++;
                    else if (v === 'линия связи') lines++;
                    else if (v === 'фонарь уличный') lamps++;
                    else if (v === 'муфта ВОЛС') muffles++;
                    else if (v === 'светильник') lanterns++;
                    else if (v === 'линии разных классов напряжения') voltLines++;
                }
                if (db.additionalLoad.communicationCabinet !== cabs || db.additionalLoad.communicationLine !== lines ||
                    ...
                    db.lamps = Array.from({ length: lamps }, () => ({ type: 'ДНАТ', customType: '', mounting: 'приставной', powerSupply: 'Кабель', direction: 'перпендикулярно линии' }));
                    changed = true;
                }
            }
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    <div class="root" id="mainPanel">
        <div class="header" id="dragHandle">
            ...
            <div class="header-controls">
                ...
                <button class="header-btn" id="btnToggleLogs" title="Журнал">📋</button>
        ...
        <div class="collapsed-info" id="collapsedInfo">
            <div class="collapsed-row">
                <span class="collapsed-label">🏗️ Опора</span>
                <span class="collapsed-value" id="collapsedPillar">—</span>
            </div>
            <div class="collapsed-row">
                <span class="collapsed-label">📐 Конструктив</span>
                <span class="collapsed-value" id="collapsedConstruct">—</span>
            </div>
        </div>
        ...
        <div class="body" id="bodyContent">
            <div class="status-panel">
                <div class="status-row">
                    <span class="status-label">🏗️ Марка опоры</span>
                    <span class="status-value" id="statusPillar">—</span>
                </div>
                <div class="status-row">
                    <span class="status-label">📐 Конструктив</span>
                    <span class="status-value" id="statusConstruct">—</span>
                </div>
            </div>
            ...
            <div class="field-group">
                <label class="field-label"><span class="field-label-icon">🏗️</span>Марка опоры</label>
                <select class="field-select" id="selPillarType"></select>
            </div>

            <div class="compact-row">
                <div class="field-group">
                    <label class="field-label"><span class="field-label-icon">📐</span>Конструктив</label>
                    <select class="field-select" id="selConstructionType"></select>
                </div>
                <div class="field-group">
                    <label class="field-label"><span class="field-label-icon">📍</span>Положение</label>
            ...
            <div class="compact-row">
                <div class="field-group">
                    <label class="field-label"><span class="field-label-icon">🔧</span>Укосы</label>
                    <div class="number-control">
                        <button class="number-btn" id="btnBrDown">−</button>
                        <div class="number-display" id="dispBracing">0</div>
                        <button class="number-btn" id="btnBrUp">+</button>
                    </div>
                </div>
                <div class="field-group">
                    <label class="field-label"><span class="field-label-icon">🪵</span>Материал</label>
            ...
            <div class="field-group">
                <label class="field-label"><span class="field-label-icon">⚡</span>Нагрузки</label>
                <div class="load-hint">ЛКМ — добавить · ПКМ — убрать</div>
                <div class="load-grid">
                    <div class="load-tile" id="loadCabinet">
                        <div class="load-tile-header">
                            <span class="load-tile-icon">🗄️</span>
                            <span class="load-tile-count" id="cntCabinet">0</span>
                        </div>
                        <div class="load-tile-name">Шкаф</div>
                    </div>
                    <div class="load-tile" id="loadCommLine">
                        <div class="load-tile-header">
                            <span class="load-tile-icon">📡</span>
                            <span class="load-tile-count" id="cntCommLine">0</span>
                        </div>
                        <div class="load-tile-name">Линия связи</div>
                    </div>
                    <div class="load-tile" id="loadStreetLamp">
                        ...
                        <div class="load-tile-name">Фонарь</div>
                    </div>
                    <div class="load-tile" id="loadVolsMuffle">
                        ...
                        <div class="load-tile-name">Муфта ВОЛС</div>
                    </div>
                    <div class="load-tile" id="loadStreetLantern">
                        ...
                        <div class="load-tile-name">Светильник</div>
                    </div>
                    <div class="load-tile" id="loadVoltageLines">
                        ...
                        <div class="load-tile-name">Разн. напр.</div>
                    </div>
                </div>
            </div>
            ...
            <div class="field-group">
                <label class="field-label"><span class="field-label-icon">👤</span>Абоненты (автозаполнение)</label>
            </div>
            <div class="field-group">
                <label class="field-label">Населенный пункт</label>
                <input type="text" class="field-input" id="inpSettlement" placeholder="Например: Опалиха">
            </div>
            <div class="field-group">
                <label class="field-label">Улица</label>
                <input type="text" class="field-input" id="inpStreet" placeholder="Например: Куйбышева пер.">
            </div>

            <div class="divider"></div>

            <div class="toggle-row" id="toggleWireTypeA">
                <span class="toggle-row-label"><span>🔩</span> Провод типа "А"</span>
            ...
            <div class="insulator-section hidden" id="insulatorSection">
                <div class="field-group">
                    <label class="field-label"><span class="field-label-icon">🔩</span>Изоляторы</label>
                </div>
                <div class="compact-row">
                    <div class="field-group">
                ...
                <div class="compact-row">
                    <div class="field-group">
                        <label class="field-label">Групп</label>
                        <div class="number-control">
                            <button class="number-btn" id="btnInsCountDown">−</button>
                            <div class="number-display" id="dispInsCount">1</div>
                            <button class="number-btn" id="btnInsCountUp">+</button>
                        </div>
                    </div>
                    <div class="field-group">
                        <label class="field-label">Кол-во в группе</label>
                        <div class="number-control">
                            <button class="number-btn" id="btnInsQtyDown">−</button>
    ...
    <div class="log-panel hidden" id="logPanel">
        <div class="log-header">
            ...
            <div class="log-actions">
                <button class="log-btn" id="btnCopyLogs">Копировать</button>
                <button class="log-btn" id="btnClearLogs">Очистить</button>
                <button class="log-btn" id="btnCloseLogs">✕</button>
            </div>
        </div>
        <div class="log-content" id="logContent">
            <div class="log-entry" style="color:rgba(255,255,255,0.35);font-style:italic;">Логи появятся здесь...</div>
        </div>
    </div>

    <div class="modal-overlay" id="modalAttach">
        <div class="modal">
            <div class="modal-title">🪵 Приставки</div>
            <div class="modal-content">
                <div class="field-group">
                    <label class="field-label">Количество</label>
                    <div class="number-control">
                        <button class="number-btn" id="btnAtDown">−</button>
            ...
            <div class="modal-buttons">
                <button class="modal-btn modal-btn-secondary" id="btnCancelAttach">Отмена</button>
                <button class="modal-btn modal-btn-primary"   id="btnConfirmAttach">Продолжить</button>
            </div>
        </div>
    </div>
    ...
    <div class="modal-overlay" id="modalLamps">
        <div class="modal">
            <div class="modal-title">💡 Настройка фонарей</div>
            <div class="modal-content" id="lampsConfig"></div>
            <div class="modal-buttons">
                <button class="modal-btn modal-btn-secondary" id="btnCancelLamps">Отмена</button>
                <button class="modal-btn modal-btn-primary"   id="btnConfirmLamps">Заполнить</button>
            </div>
        </div>
    </div>
    `;
    ...
    function log(message, type = 'info') {
        ...
        const icons  = { success: '✅', error: '❌', warning: '⚠️', debug: '🔍', info: 'ℹ️' };
        const el = document.createElement('div');
        el.className = 'log-entry';
        el.style.color = colors[type] || colors.info;
        el.innerHTML = `<span style="opacity:0.5;">[${ts}]</span> ${icons[type]||'ℹ️'} ${message}`;
        ui.logContent.appendChild(el);
        ui.logContent.scrollTop = ui.logContent.scrollHeight;
    }
    function clearLogs() {
        logMessages = [];
        ui.logContent.innerHTML = '<div class="log-entry" style="color:rgba(255,255,255,0.35);font-style:italic;">Очищено</div>';
    }
    function copyLogs() {
        ...
        t.select();
        try { document.execCommand('copy'); showNotification('✅ Логи скопированы!'); }
    ...
    ui.selPillar.addEventListener('change', e => {
        ...
        db.pillar.material = PILLAR_TYPE_TO_MATERIAL[e.target.value] || 'железобетонная';
    ...
    [
        ...
        el.addEventListener('click', e => {
            ...
            if (key === 'streetLamp') db.lamps.push({ type: 'ДНАТ', customType: '', mounting: 'приставной', powerSupply: 'Кабель', direction: 'перпендикулярно линии' });
            saveDb(); updateUI();
    ...
    /******************************************************************
     * ГЕНЕРАЦИЯ UI ДЛЯ ФОНАРЕЙ
     ******************************************************************/
    function genLampsUI() {
        ui.lampsConfig.innerHTML = db.lamps.map((lamp, i) => `
            <div class="lamp-card">
                <div class="lamp-card-title">💡 Фонарь ${i + 1}</div>
                <div class="field-group" style="margin-bottom:8px;">
                    <label class="field-label">Тип лампы</label>
                    <select class="field-select lamp-type" data-i="${i}">
                        ${LAMP_TYPES.map(t => `<option value="${t}" ${lamp.type===t?'selected':''}>${t}</option>`).join('')}
                    </select>
                </div>
                ${lamp.type === 'Ввод вручную' ? `
                <div class="field-group" style="margin-bottom:8px;">
                    <label class="field-label">Введите тип</label>
                    <input type="text" class="field-input lamp-custom" data-i="${i}" value="${lamp.customType||''}" placeholder="Например: ДРВ-250">
                </div>` : ''}
                <div class="field-group" style="margin-bottom:8px;">
                    <label class="field-label">Крепление</label>
                    <div class="radio-group">
                        <div class="radio-option"><input type="radio" class="radio-input lamp-mount" id="m${i}a" name="m${i}" value="приставной"  ${lamp.mounting==='приставной' ?'checked':''} data-i="${i}"><label class="radio-label" for="m${i}a">Приставной</label></div>
                        <div class="radio-option"><input type="radio" class="radio-input lamp-mount" id="m${i}b" name="m${i}" value="консольный" ${lamp.mounting==='консольный'?'checked':''} data-i="${i}"><label class="radio-label" for="m${i}b">Консольный</label></div>
                    </div>
                </div>
                <div class="field-group" style="margin-bottom:8px;">
                    <label class="field-label">Питание</label>
                    <div class="radio-group">
                        <div class="radio-option"><input type="radio" class="radio-input lamp-power" id="p${i}a" name="p${i}" value="Кабель" ${lamp.powerSupply==='Кабель'?'checked':''} data-i="${i}"><label class="radio-label" for="p${i}a">Кабель</label></div>
                        <div class="radio-option"><input type="radio" class="radio-input lamp-power" id="p${i}b" name="p${i}" value="Провод" ${lamp.powerSupply==='Провод'?'checked':''} data-i="${i}"><label class="radio-label" for="p${i}b">Провод</label></div>
                    </div>
                </div>
                <div class="field-group">
                    <label class="field-label">Направление</label>
                    <select class="field-select lamp-dir" data-i="${i}">
                        <option value="перпендикулярно линии" ${lamp.direction==='перпендикулярно линии'?'selected':''}>Перпендикулярно линии</option>
                        <option value="произвольно"           ${lamp.direction==='произвольно'          ?'selected':''}>Произвольно</option>
                        <option value="параллельно линии"     ${lamp.direction==='параллельно линии'    ?'selected':''}>Параллельно линии</option>
                    </select>
                </div>
            </div>`).join('');
    ...
    ui.btnScan.addEventListener('click', async () => {
        ...
        try {
            await loadFromForm();
            showNotification('✅ Данные считаны с сайта', 'success');
        } catch (err) {
    ...
    ui.btnFill.addEventListener('click', async () => {
        clearLogs();
        clearAllErrors();
        log('🚀 Запуск заполнения опоры...', 'info');
        if (db.pillar.material === 'деревянная') {
    ...
    async function startFill() {
        ...
        showNotification('⏳ Заполнение...', 'warning');
        try {
            log('═'.repeat(45), 'info');
            log('v1.8.4 — Заполнение опоры', 'info');
            log('═'.repeat(45), 'info');
            ...
            if (db.pillar.type) {
                ...
                log(`Марка: ${db.pillar.type}`, 'success');
                await sleep(150);
            }

            await fillField('input[name="constructionType"]', db.pillar.constructionType, 'Конструктив');
            await sleep(150);
            await fillField('input[name="placement"]', db.pillar.placement, 'Местоположение');
            await sleep(150);
            await fillField('input[name="material"]', db.pillar.material, 'Материал');
            await sleep(150);

            if (db.pillar.material === 'деревянная')
                await fillField('input[name="woodTemporaryType"]', 'без обработки', 'Консервация');
            await sleep(150);

            log(`\n=== СТОЙКИ (${db.stands.count}) ===`, 'info');
            if (await setCounter('input[name="stand.count"]', db.stands.count) && db.stands.count > 0) {
                await sleep(600);
                for (let i = 0; i < db.stands.count; i++) {
                    log(`Стойка ${i+1}/${db.stands.count}`, 'info');
                    await goToSlide(i); await sleep(200);
                    await fillField(`input[name="stand.info[${i}].material"]`, db.stands.material, `Мат. стойки ${i+1}`);
                    await sleep(100);
            ...
            if (db.bracing.count > 0) {
                log(`\n=== УКОСЫ (${db.bracing.count}) ===`, 'info');
                if (await setCounter('input[name="bracing.count"]', db.bracing.count)) {
                    await sleep(600);
                    for (let i = 0; i < db.bracing.count; i++) {
                        log(`Укос ${i+1}/${db.bracing.count}`, 'info');
                        await goToSlide(i); await sleep(200);
                        await fillField(`input[name="bracing.info[${i}].material"]`, db.bracing.material, `Мат. укоса ${i+1}`);
                        await sleep(100);
            ...
            if (db.pillar.material === 'деревянная' && db.attachments.count > 0) {
                log(`\n=== ПРИСТАВКИ (${db.attachments.count}) ===`, 'info');
                if (await setCounter('input[name="attachment.count"]', db.attachments.count)) {
                    await sleep(600);
                    for (let i = 0; i < db.attachments.count; i++) {
                        log(`Приставка ${i+1}/${db.attachments.count}`, 'info');
                        await goToSlide(i); await sleep(200);
                        await fillField(`input[name="attachment.info[${i}].material"]`, db.attachments.material, `Мат. прист. ${i+1}`);
                        await sleep(100);
    ...
    ui.btnMin.addEventListener('click', () => {
        ...
        ui.btnMin.textContent = db.ui.isCollapsed ? '+' : '−';
        saveDb();
    });
    ...
    /******************************************************************
     * ГОРЯЧАЯ КЛАВИША ALT+О
     ******************************************************************/
    window.pillarManuallyHidden = false;
    document.addEventListener('keydown', e => {
        if (e.altKey && e.key.toLowerCase() === 'о') {
            e.preventDefault();
            window.pillarManuallyHidden = !window.pillarManuallyHidden;
            checkFormPresence();
            const msg = window.pillarManuallyHidden ? '🙈 Опоры скрыты. ALT+О для показа' : '✅ Опоры показаны. ALT+О для скрытия';
            const n = document.createElement('div');
    ...
    console.log('✅ Автозаполнение опор v1.8.4 готов! | Created by KAST Team');
    console.log('💡 Горячая клавиша: ALT+О — показать/скрыть UI');
})();
